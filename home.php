 ...
 <div class="container">
    ...
    <div class="row">
      ...
      <i class="icon-calendar icon-large"></i>
                <?php
                $today = date('Y-m-d');
                $new = date('l, F d, Y - h:i A', strtotime($today));
                echo $new;
     ...
     <form name="clock" id="clock" method="POST" action="#"><!--*****Clock******-->
   ...
   <div class="container">
     <div class="row">
      <div class="contentheader">
        <h2>Accueil - Gérer les ventes</h2>
       </div> <hr>
     
    <center>
      <div class="col-md-12">
        <form method="POST" action="insert_invoice.php?invoice_number=<?php echo $_GET['invoice_number']?> " >
         <input type="text" name="bar_code" id="bar_code" autocomplete="off" placeholder="Entrez le numéro de code-barres" style="width:300px;height: 30px;">
     <input type="text" id="product" required  autocomplete="off" placeholder="Médicament" style="width:400px;height: 30px;">
     <!-- <div class="ui-widget"> -->
     <input type="hidden" name="product" id="product_hidden" required class="form-control" autocomplete="off" placeholder="Médicament" style="width:400px;height: 30px;">
   <!-- </div> -->
      <input type="hidden" name="expire_date" id="date_hidden" required class="form-control" autocomplete="off" placeholder="Médicament" style="width:400px;height: 30px;">
     <input type="number" name="avai_qty" id="avai_qty"  readonly placeholder="Quantité disponible" style="width: 100px; height:30px;">
   
     <input type="number" name="qty" id="qty"  placeholder="Quantité" autocomplete="off"  style="width: 100px; height:30px;" required>
    ...
    <div class="row">
      <table class="table table-bordered table-striped table-hover" id="resultTable" data-responsive="table">
        ...
        <tr style="background-color: #383838; color: #FFFFFF;" >
            <th> Médicament </th>
            <th> Catégorie</th>
            <th style="background: #c53f3f;"> Date d'expiration</th>
            <th> Prix </th>
            <th> Quantité </th>
            <th> Montant </th>
            <th> Action </th>
           ...
           <td><a href="delete_invoice.php?invoice_number=<?php echo $_GET['invoice_number']?>&id=<?php echo $row['id'];?>&name=<?php echo $row['medicine_name']?>&expire_date=<?php echo $row['expire_date']?>&quantity=<?php echo $row['qty'];?>" class="btn btn-danger">Retirer</a></td>
          ...
          <?php
           if($medicine_name && $category && $quantity !=null){
            ...
            <a id="popup" href="checkout.php?invoice_number=<?php echo $_GET['invoice_number']?>&medicine_name=<?php echo $medicine_name?>&category=<?php echo $category?>&ex_date=<?php echo $ex_date?>&quantity=<?php echo $quantity?>&total=<?php echo $grand_total?>&profit=<?php echo $grand_profit?>" style="width:400px;" class="btn btn-info btn-large">Procéder <i class="icon icon-share-alt"></i></a>
      ...
      <div class="alert alert-danger">
        <h3><center>Aucune vente disponible !!</center> </h3>
    </div>
